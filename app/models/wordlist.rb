class Wordlist < ApplicationRecord
  ...
  scope :category_filter, -> (name){ joins(:category).where("categories.name = ? and extract(year from wordlists.created_at) = 2021 ",name)}
  ...
  def self.filter name
    if name
      ...
      category_filter(Category.first.name).group(:name).group_by_week(:created_at).count
    end
  end
end
