class Admin::ContentLessonsController < ApplicationController
  ...
  def index
    ...
    return @content_lessons = @lesson.content_lessons.search(params[:name]).paginate(page: params[:page]) unless @lesson.nil?
    flash[:danger] = "Not found Lesson"
  ...
  def destroy
    if @content.destroy
      ...
      flash[:success] ="Delete unsuccess"
      redirect_to request.referer
    end
  end
  private
    def found_content_lesson
      ...
      flash[:danger] = "Not found content lessons"
      redirect_to admin_lessons_path
    end 
    def params_word
